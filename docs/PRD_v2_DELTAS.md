...
| **§7.7** LLM-emitted structured tags | Regex-based detection for hardship, PTP capture, refusal — every demo surfaced a phrasing the regex missed; the temptation was to add another regex pattern. | Five required tags now emitted with every bot reply: `[MOVE: X]`, `[CUSTOMER_HARDSHIP: bool]`, `[CUSTOMER_PTP_CAPTURED: bool]`, `[CUSTOMER_WANTS_TO_END: bool]`, `[END_CALL: bool]`. Same `[NAME: value]` shape; LLM declares understanding, deterministic code derives consequences. Tag fields recorded in audit JSONL; missing tags fall back gracefully. | The architectural answer to symptom-fix-via-regex. ~80% of the 250-pattern regex bank becomes removable in v3 on this pattern (intent classifier and commitment validator). |
| **§7.8** Closing-turn bidirectional coherence | Bot's spoken words ("Take care") and structured close tag could disagree — customer heard a question while the system closed, or heard "closing now" while the system kept the call open. | base.txt rule: (a) IF `[END_CALL: true]` → no question allowed in reply; (b) IF reply contains a closing phrase → MUST set `[END_CALL: true]`. Words and tag must agree. | Customer asking "did you close the call?" after the bot said "closing the call now" was a real recorded transcript. Contract enforcement at the prompt layer. |
| **§7.9** `[END_CALL]` guard loosened after structural layers stabilised | Initial v2 guard demanded `terminal_outcome` be set via approved routes; if none caught the customer's intent, the LLM's `[END_CALL: true]` was stripped and the bot looped. | Past the opener, the LLM's `[END_CALL: true]` is honoured. The other six layers (move ladder, speech authority, closing-turn coherence, four structured tags, validator) subsumed the guard's protective function — keeping the guard meant the bot lied to customers ("Closing the call now" while system stayed open). | Trust adjustment between layers, not a removal of protection. Logged as `guard:llm_authorised_close_late` for monthly compliance review. |
...
| **Bot fault-tolerance / malfunction containment** — when validator blocks 2+ consecutive turns, or 2+ consecutive turns produce near-identical output, transition to a new `SYSTEM_DIFFICULTY_CLOSE` state and exit gracefully ("My apologies, I'm having trouble on my end. The helpline is open whenever you're ready"). Outcome posted as `human_callback_required / system_difficulty` with transcript attached for Ops review. **The customer never experiences a stuck bot.** | `conversation.py` (consecutive-failure counter), `fsm.py` (new state), `prompts/closes/system_difficulty.txt` (new template), `app/outcome/schema.py` (new reason enum value) | Yes — additive. Existing behaviour preserved when bot is healthy. |
| **Validator paradigm shift — structured-output validation** — replace regex-based commitment-overreach detection (currently 12 patterns, brittle; every demo surfaces an edge case) with the LLM declaring its commitments in a structured field (`[BOT_COMMITMENTS: <list>]` tag), and code validating those declarations against an allow-list. Same architectural pattern as the four LLM-emitted signal tags already shipped. Eliminates the "fight between bot's natural language and the regex wall" failure mode that produced the P26 'are you idiot' loop. | `prompts/base.txt` (new required tag), `app/conversation.py` (extractor), `app/validator.py` (allow-list check) | Mostly. The regex bank would shrink dramatically; validator becomes a thin allow-list checker rather than 88 patterns. |

These are additive. The current 42-scenario eval baseline remains valid; v3 work would add ~10 new scenarios specifically testing repeat-call behaviour (Attempt 2 with a broken PTP, Attempt 3 with no history, hardship-after-second-attempt, etc.) plus ~5 fault-injection scenarios that deliberately trigger the malfunction-containment path.

**Architectural principle behind the fault-tolerance row:** *bot bugs should never manifest as customer-facing experience.* Currently they do — the P26 transcript showed a validator over-fire causing the customer to hear the same fallback three times and react with hostility. That's blast-radius from an internal bug reaching the user. The fault-tolerance layer is a circuit breaker — same shape as the FSM's other terminal exits, but for bot health rather than customer state. Pulled out of v2 scope as the user's call; documented here so it doesn't get lost.
