...
| `refused` (reason=`refused_current_call`) **[NEW — Layer 3]** | In-the-moment refusal of THIS call, caught by the regex `refuse_current_call` intent (two-strike). **Not** TRAI DND. Cooldown then Call 2 with a different opener. | Per `outcome_detail.callback_sla_hours` (24–72h based on segment) | Per max-attempts rule |
| `refused` (reason=`customer_signaled_end`) **[NEW — LLM tag]** | LLM detected close-intent via `[CUSTOMER_WANTS_TO_END: true]` — catches implicit refusals the regex missed ("end of discussion", "what else do you need", "I've told you"). Same retry semantics as `refused_current_call`. | Per `outcome_detail.callback_sla_hours` | Per max-attempts rule |
| `refused` (reason=`ladder_exhausted_no_capture`) **[NEW — Layer 1]** | FSM played every move in the ladder; customer didn't converge. Bot has tried everything legitimate; further turns add nothing. Distinct from hostile refusal (no abuse signal raised). | Per `outcome_detail.callback_sla_hours` (typically 48–72h) | Per max-attempts rule; possibly a more thoughtful retry approach (Call 2 with different strategy modifier) |
